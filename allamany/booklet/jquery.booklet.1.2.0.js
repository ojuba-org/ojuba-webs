		...
		if(typeof options == 'string')
		{
			//check if booklet has been initialized
			 if($(this).data('booklet')){
				...
				if(command == 'next'){ obj.prev() }
				else if(command == 'prev'){ obj.next() }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(opts.shadows){
		opts.shadowTopBackWidth  = '-'+opts.shadowTopFwdWidth+'px';
		opts.shadowTopFwdWidth  = '-'+opts.shadowTopBackWidth+'px';
	}
		...
		gotoPage : function(num){
			...
			if(num > opts.curr && num < opts.pTotal && num >= 0 && !busy){
				...
				if(opts.closed && opts.autoCenter && num-diff == 0){
					p2.stop().animate({width:0, right:opts.pWidth}, opts.speed, opts.easing);
					p4.stop().animate({right:opts.pWidth}, opts.speed, opts.easing);
				}else{
				...
				if(p3drag){
					p3.stop().animate({right:opts.pWidth/4, width:opts.pWidth*.75, paddingLeft: opts.shadowBtmWidth}, opts.speedH, opts.easeIn)
					         .animate({right:0, width:opts.pWidth, paddingLeft:0}, opts.speedH);
				}else{
					p3.stop().animate({right:opts.pWidthH, width:opts.pWidthH, paddingLeft: opts.shadowBtmWidth}, opts.speedH, opts.easeIn)
					         .animate({right:0, width:opts.pWidth, paddingLeft:0}, opts.speedH);			
				}
				p3wrap.animate({right:opts.shadowBtmWidth}, opts.speedH, opts.easeIn)
			...
			//moving backward (decreasing number)
				...
				if(p0drag){
					...
					p1.animate({right:opts.pWidth, width:0}, opts.speed, opts.easing);
					p1wrap.animate({right:opts.pWidthN}, opts.speed, opts.easing);
					...
					if(opts.closed && opts.autoCenter && opts.curr == 0){
						p0.animate({right:opts.pWidthH, width:opts.pWidthH}, opts.speedH, opts.easeIn)
						  .animate({right:0, width:opts.pWidth}, opts.speedH, opts.easeOut);
						p2.stop().animate({right:0}, opts.speed, opts.easing);
					}else{
						p0.animate({right:opts.pWidth, width:opts.pWidth}, opts.speed, opts.easing);
					}
					//animate .wrapper content with p0 to keep content right aligned throughout
					//call setuppages at end of animation to reset pages
					p0wrap.animate({left:0}, opts.speed, opts.easing, function(){updateAfter()});						
				}else{
					//hide p1 as p0 moves across it
					p1.animate({right:opts.pWidth, width:0}, opts.speed, opts.easing);
					p1wrap.animate({right:opts.pWidthN}, opts.speed, opts.easing);
					...
					if(opts.closed && opts.autoCenter && opts.curr == 0){
						...
						p0.animate({right:opts.pWidthH, width:opts.pWidthH}, opts.speedH, opts.easeIn)
						  .animate({right:opts.pWidth, width:opts.pWidth}, opts.speedH, opts.easeOut);
					}
					//animate .wrapper content with p0 to keep content right aligned throughout
					//call setuppages at end of animation to reset pages
					p0wrap.animate({left:opts.shadowBtmWidth}, opts.speedH,opts. easeIn)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//add overlays
	if(opts.overlays){
		overlayN = $('<div class="b-overlay b-overlay-prev b-prev" title="الصفحة التالية"></div>').appendTo(b);
		overlayP = $('<div class="b-overlay b-overlay-next b-next" title="الصفحة السابقة"></div>').appendTo(b);
	...
	//add tabs
	if(opts.tabs){
		tabN = $('<div class="b-tab b-tab-prev b-prev" title="Previous Page">Previous</div>').appendTo(b);
		tabP = $('<div class="b-tab b-tab-next b-next" title="Next Page">Next</div>').appendTo(b);
		tabs = b.find('.b-tab');
		...
		if(opts.direction == 'RTL'){
			tabP.html('Previous').attr('title','الصفحة التالية');
			tabN.html('Next').attr('title','الصفحة السابقة');
		}
	}else{
		b.css({'marginTop': 0});
	}
	...
	//add arrows
	if(opts.arrows){
		arrowN = $('<div class="b-arrow b-arrow-prev b-prev" title="الصفحة التالية"><div>Previous</div></div>').appendTo(b);
		arrowP = $('<div class="b-arrow b-arrow-next b-next" title="الصفحة السابقة"><div>Next</div></div>').appendTo(b);
		arrows = b.find('.b-arrow');
		
		//update ctrls for RTL direction
		if(opts.direction == 'RTL'){
			arrowP.html('<div>Previous</div>').attr('title','Previous Page');
			arrowN.html('<div>Next</div>').attr('title','Next Page');
		}
	}
	
	//save all "b-prev" and "b-next" controls
	ctrlsP = b.find('.b-next');
	ctrlsN = b.find('.b-prev');
	
	//add click actions
	ctrlsP.click(function(e){e.preventDefault(); self.next();});
	ctrlsN.click(function(e){e.preventDefault(); self.prev();});
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function resetCSS(){
		...
		wraps.attr('style','');
		wraps.css({'right':0,'width':opts.pWidth-(opts.pagePadding*2), 'height':opts.pHeight-(opts.pagePadding*2), 'padding': opts.pagePadding});
		p0wrap.css({'left':0,'right':'auto'});
		p1.css({'right':0,'width':opts.pWidth, 'height':opts.pHeight});			
		p2.css({'right':opts.pWidth, 'width':opts.pWidth, 'opacity':1, 'height':opts.pHeight});
		pN.css({'right':0, 'width':opts.pWidth, 'height':opts.pHeight});
		p0.css({'right':0, 'width':0, 'height':opts.pHeight});
		p3.stop().css({'right':opts.pWidth*2, 'width':0, 'height':opts.pHeight, paddingLeft:0});
		p4.css({'right':opts.pWidth, 'width':opts.pWidth, 'height':opts.pHeight});
		
		if(opts.closed && opts.autoCenter && opts.curr == 0){
			pN.css({'right':0});
			p1.css({'right':opts.pWidthN});
			p2.css({'right':0});
			p3.css({'right':opts.pWidth});
			p4.css({'right':0});
		}
	...
	function resetPages(){	
		...
		if(opts.shadows){
			...
			sF = $('<div class="b-shadow-f"></div>').css({'left':0,'width':opts.pWidth, 'height':opts.pHeight}).appendTo(p3);
			sB = $('<div class="b-shadow-b"></div>').appendTo(p0).css({'right':0,'width':opts.pWidth, 'height':opts.pHeight});		
		}
		...
		if(opts.manual && $.ui){
			...
			p3.draggable({
				axis: "x",
				containment: [p2.offset().right-opts.pWidthH,0,p2.offset().right+opts.pWidth-50,opts.pHeight],
				drag: function(event, ui) {
					p3drag = true;
					temp = ui.originalPosition.right - ui.position.right;
					p3.removeClass('b-grab').addClass('b-grabbing');
					p3.width(40+(temp/2));
					p3wrap.css({'right':10+(temp/8)});
					p2.width(opts.pWidth-p3.width()+10);
					if(opts.shadows){
						sF.css({'left':'-'+(20+temp/4)+'px'});
						if($.support.opacity){
							sF.css({'opacity':0.5*(temp/opts.pWidthH)});
						}else{
							sF.css({'left':'auto','left':0.1*p3.width()});
						}
					}
				},
				stop: function(event, ui) {
					hoverAnimEnd(false);
					var temp = ui.originalPosition.right - ui.position.right;
			...
			p0.draggable({
				axis: "x",
				containment: [p1.offset().right+10,0,p1.offset().right+opts.pWidth*.75,opts.pHeight],				
				drag: function(event, ui) {
					p0drag = true;
					temp = ui.position.right - ui.originalPosition.right;
					p0.removeClass('b-grab').addClass('b-grabbing');
					p0.css({right:40+(temp)/1.5, width:40+(temp)});
					p0wrap.css({left:10+temp/4});	
					p1.css({right:ui.position.right+20, width:opts.pWidth-ui.position.right-10});			
					p1wrap.css({right:-1*(temp+30)});	
					if(opts.shadows){
						if($.support.opacity){
							sB.css({'opacity':0.5*(temp/opts.pWidthH)});
						}else{
							sB.css({'right':-0.38*opts.pWidth});
						}
					}
				},
				stop: function(event, ui) {
			...
			$(b).unbind('mousemove mouseout').bind('mousemove',function(e){
				relativeX = e.pageX - b.offset().right;
				if(relativeX < 50){
					hoverAnimStart(false);
	...
	function hoverAnimStart(inc){
		if(inc){
			if(!busy && !rhover &&!lhover && !p3drag && opts.curr+2 <= opts.pTotal-2){
				...
				p3.addClass('b-grab');
				if(opts.closed && opts.autoCenter && opts.curr == 0){
					p3.stop().animate({'right':opts.pWidth-50, 'width':40}, 500, opts.easing);
				}else{
					p3.stop().animate({'right':opts.width-50, 'width':40}, 500, opts.easing);
				}
				p3wrap.stop().animate({'right':10}, 500, opts.easing);
				if(opts.shadows && !$.support.opacity){
					sF.css({'left':'auto','right':'-40%'});
				}
				rhover = true;
			}
		}else{
			if(!busy && !lhover && !rhover && !p0drag && opts.curr-2 >= 0){
				//animate
				p1.stop().animate({right:10, width:opts.pWidth-10}, 400, opts.easing);
				p0.addClass('b-grab');
				p1wrap.stop().animate({right:"-10px"}, 400, opts.easing);
				p0.stop().animate({right:10, width:40}, 400, opts.easing);
				p0wrap.stop().animate({left:10}, 400, opts.easing);
				if(opts.shadows && !$.support.opacity){
					sB.css({'right':-0.38*opts.pWidth});
				}
				lhover = true;
	...
	function hoverAnimEnd(inc){
		if(inc){
			if(!busy && rhover && !p3drag && opts.curr+2 <= opts.pTotal-2){
				...
				if(opts.closed && opts.autoCenter && opts.curr == 0){
					p3.stop().animate({'right':opts.pWidth, 'width':0}, 500, opts.easing);				
				}else{
					p3.stop().animate({'right':opts.width, 'width':0}, 500, opts.easing);	
				}
				p3wrap.stop().animate({'right':0}, 500, opts.easing);
				if(opts.shadows && !$.support.opacity){
					sF.css({'right':'auto'});
				}
				rhover = false;
			...
			if(!busy && lhover && !p0drag && opts.curr-2 >= 0){
				p1.stop().animate({right:0, width:opts.pWidth}, 400, opts.easing);
				p1wrap.stop().animate({right:0}, 400, opts.easing);
				p0.stop().animate({right:0, width:0}, 400, opts.easing);
				p0wrap.stop().animate({left:0}, 400, opts.easing);
				lhover = false;
			}
		}
	}
	...
	function initAnim(diff, inc, shadow){			
		...
		if(inc && diff > 2){
			...
			b.find('.b-page-'+opts.curr).addClass('b-p3').show().stop().css({'right':opts.pWidth*2, 'width':0, 'height':opts.pHeight, paddingLeft:0});
			b.find('.b-page-'+(opts.curr+1)).addClass('b-p4').show().css({'right':opts.pWidth, 'width':opts.pWidth, 'height':opts.pHeight});
			...
			if(rhover){
				p3.css({'right':opts.width-40, 'width':20, 'padding-right': 10});
			}
			...
			b.find('.b-page-'+opts.curr).addClass('b-pN').show().css({'right':0, 'width':opts.pWidth, 'height':opts.pHeight});
			b.find('.b-page-'+(opts.curr+1)).addClass('b-p0').show().css({'right':0, 'width':0, 'height':opts.pHeight});
			b.find('.b-page-'+opts.curr+' .b-wrap').show().css({'width':opts.pWidth-(opts.pagePadding*2), 'height':opts.pHeight-(opts.pagePadding*2), 'padding': opts.pagePadding});
			b.find('.b-page-'+(opts.curr+1)+' .b-wrap').show().css({'width':opts.pWidth-(opts.pagePadding*2), 'height':opts.pHeight-(opts.pagePadding*2), 'padding': opts.pagePadding});
		...
		if(opts.shadows){
			...
			if($.support.opacity){
				...
				if(inc){
					shadow.animate({left:opts.shadowTopFwdWidth}, opts.speed, opts.easeIn);
				}else{
					shadow.animate({right:opts.shadowTopBackWidth}, opts.speed, opts.easeIn);
				}
			}
		}
...
$.fn.booklet.defaults = {
	...
	hovers:             1,                            // enables preview pageturn hover animation, shows a small preview of previous or next page on hover
	overlays:           1,                            // enables navigation using a page sized overlay, when enabled links inside the content will not be clickable
	...
	auto:               0,                           // enables automatic navigation, requires "delay"
